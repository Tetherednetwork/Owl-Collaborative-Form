...
export const Header: React.FC<HeaderProps> = ({ onHomeClick, actions }) => {
    const { currentUser, logout } = useAuth();
    
    if (!currentUser) {
        return (
            <header className="bg-white/70 backdrop-blur-lg sticky top-0 z-10">
        ...
        <header className="bg-[#272458] text-white shadow-lg sticky top-0 z-10">
            <div className="container mx-auto px-4 md:px-8 py-4 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <button onClick={onHomeClick} aria-label="Go to homepage">
                        <OwlLogo />
                    </button>
                </div>
